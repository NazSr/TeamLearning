...
```bash
OrdersApi$ sam logs -n HelloEfsFunction --stack-name OrdersApi --tail
```
...
```bash
OrdersApi$ pip install -r tests/requirements.txt --user
# unit test
OrdersApi$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
OrdersApi$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
